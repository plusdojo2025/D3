...
public class TalkDAOTest {
    public static void main(String[] args) {
        ...
        // SELECT テスト
        Talk searchCondition = new Talk(null, 101, null); // topic_id 101 を持つトークを検索
        List<Talk> resultList = dao.select(searchCondition);
        System.out.println("検索結果:");
        if (resultList != null && !resultList.isEmpty()) {
            for (Talk t : resultList) {
                System.out.println("Customer ID: " + t.getCustomer_id()
                        + ", Topic ID: " + t.getTopic_id()
        ...
        // DELETE テスト
        Talk deleteTalk = new Talk(1, null, null); // customer_id = 1 を削除対象とする
